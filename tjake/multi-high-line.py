...
def single_move(p0, q0, sw):
    ...
    if sw:
        ...
        while abs(y1 - y0) > 0:
            if y0 < y1:
                cs.append(SMove((0, 0, LD)))
                y0 += LD
            else:
                cs.append(SMove((0, 0, -LD)))
                y0 -= LD
        #print(">", x0, y0, x1, y1)
        if abs(x1 - x0) <= SD and abs(y1 - y0) <= SD:
            if x0 != x1 or y0 != y1:
                if x0 == x1 and y0 != y1:
                    cs.append(SMove((0, 0, y1 - y0)))
                elif x0 != x1 and y0 == y1:
                    cs.append(SMove((x1 - x0, 0, 0)))
                else:
                    cs.append(LMove((x1 - x0, 0, 0), (0, 0, y1 - y0)))
        else:
            if x0 != x1:
                cs.append(SMove((x1 - x0, 0, 0)))
            if y0 != y1:
