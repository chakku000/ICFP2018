...
std::vector<command> compress(const std::vector<command> cmds){
    int x=0,y=0,z=0;
    std::vector<command> ret;
    for(size_t i=0;i<cmds.size();i++){
        if(cmds[i].type == OP_SMOV or cmds[i].type == OP_LMOV){
            x += cmds[i].data.lld.x;
            y += cmds[i].data.lld.y;
            z += cmds[i].data.lld.z;
        }else{
            ...
            while(x){
                int nx = std::min(15,x);
                ret.emplace_back(opSMov(nx*tx,0,0));
                x -= nx;
            }
            while(y){
                int ny = std::min(15,y);
                ret.emplace_back(opSMov(0,ny*ty,0));
                y -= ny;
            }
            while(z){
                int nz = std::min(15,z);
                ret.emplace_back(opSMov(0,0,nz*tz));
                z -= nz;
            }
